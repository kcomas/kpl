...
static void rloop(uint8_t *m) {
    ...
    jit_push(&p, m, R(10));
    size_t lbl = p;
    jit_movrq(&p, m, R(AX), P(&printf));
    jit_movrq(&p, m, R(DI), P("V: %ld\n"));
