...
void btest(void) {
    ...
    lst_stop->d[0] = U6(tkn_a(t, TCUST(RS), "]", &tkn_ft));
    psr_a(p, PARSER(UN), PSR_MODE(STOP), NULL, NULL, NULL, NULL, 1, U6(tkn_a(t, TCUST(SEMI), ";", &tkn_ft)));
    psr_a(p, PARSER(UN), PSR_MODE(STOP), NULL, NULL, NULL, NULL, 1, U6(tkn_a(t, TCUST(NL), "\n", &tkn_ft)));
    ...
    psr_a(p, PARSER(UN), PSR_MODE(LOOP), te_c(lst_stop), &psr_lst_e, &psr_val_m, &psr_lst_i, 1, tkn_a(t, TCUST(LS), "[", &tkn_ft));
    tkn_p(t->t, 0);
    psr_p(p->pt, 0);
    printf("%s\n", pgm);
    psr_stat pstat;
