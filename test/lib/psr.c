...
void btest(void) {
    ...
    psr_a(p, PCUST(INT), PSR_MODE(ONCE), te_c(stmt_stop), &psr_val_m, &psr_int_i, 1, TCUST(NUM));
    ...
    psr_a(p, PCUST(FLT), PSR_MODE(ONCE), te_c(stmt_stop), &psr_val_m, &psr_flt_i, 3, TCUST(NUM), tkn_a(t, TOKEN(UN), ".", &tkn_ft), TCUST(NUM));
    psr_p(p->pt, 0);
