...
LST = lst$(TNAME)
LLST_OBJS = $(LSRC)/lst.o $(LTE_OBJS) $(LMC_OBJS)
OBJS += $(LLST_OBJS)
...
GEN_TYPE = gen_type$(TNAME)
GEN_TYPE_OBJS = $(LSRC)/gen_type.o $(GEN_OBJS) $(GENX64_OBJS) $(TYPE_OBJS)
OBJS += $(GEN_TYPE_OBJS)
$(GEN_TYPE): $(GEN_TYPE_OBJS) $(TEST)/gen_type.o $(TEST)/gen_t.o $(TEST)/as_t.o $(TEST_OBJS)
> $(CCOBJ)
...
NS = ns$(TNAME)
NS_OBJS = $(SRC)/ns.o $(TYPE_OBJS) $(LERR_OBJS) $(LVR_OBS) $(patsubst %.c,%.o,$(wildcard $(SRC)/ns/*.c))
OBJS += $(NS_OBJS)
...
ATG_OBJS = $(LSRC)/atg.o $(OPT_OBJS) $(GEN_TYPE_OBJS)
