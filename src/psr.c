...
psr *psr_b(psr *p) {
    ...
    psr_a(p, PCUST(TYPE), PSR_MODE(ONCE), NULL, NULL, &psr_val_m, &psr_type_i, 1, tkn_a(p->tt, TOKEN(UN), "I6", &tkn_ft));
    psr_a(p, PCUST(TYPE), PSR_MODE(ONCE), NULL, NULL, &psr_val_m, &psr_type_i, 1, tkn_a(p->tt, TOKEN(UN), "U6", &tkn_ft));
    psr_a(p, PCUST(TYPE), PSR_MODE(ONCE), NULL, NULL, &psr_val_m, &psr_type_i, 1, tkn_a(p->tt, TOKEN(UN), "F6", &tkn_ft));
    psr_a(p, PCUST(TYPE), PSR_MODE(ONCE), NULL, NULL, &psr_val_m, &psr_type_i, 1, tkn_a(p->tt, TOKEN(UN), "FN", &tkn_ft));
    psr_a(p, PCUST(TYPE), PSR_MODE(ONCE), NULL, NULL, &psr_val_m, &psr_type_i, 1, tkn_a(p->tt, TOKEN(UN), "UN", &tkn_ft));
    psr_a(p, PCUST(SELF), PSR_MODE(ONCE), NULL, NULL, &psr_val_m, &psr_key_i, 1, tkn_a(p->tt, TOKEN(UN), "S", &tkn_ft));
    psr_a(p, PCUST(SUM), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), "Σ", &tkn_ft));
    psr_a(p, PCUST(SUB), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), "-", &tkn_ft));
    psr_a(p, PCUST(ADD), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), "+", &tkn_ft));
    psr_a(p, PCUST(EQ), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), "=", &tkn_ft));
    psr_a(p, PCUST(LT), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), "<", &tkn_ft));
    psr_a(p, PCUST(GT), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), ">", &tkn_ft));
    psr_a(p, PCUST(AND), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), "&", &tkn_ft));
    psr_a(p, PCUST(MH), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), "#", &tkn_ft));
    psr_a(p, PCUST(AGN), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), ":", &tkn_ft));
    psr_a(p, PCUST(CST), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), "$", &tkn_ft));
    psr_a(p, PARSER(UN), PSR_MODE(ONCE), NULL, NULL, NULL, NULL, 1, TCUST(WS));
    psr_a(p, PCUST(FLT), PSR_MODE(ONCE), NULL, NULL,  &psr_val_m, &psr_flt_i, 3, TCUST(NUM), tkn_a(p->tt, TOKEN(UN), ".", &tkn_ft), TCUST(NUM));
    ...
    psr_a(p, PCUST(IF), PSR_MODE(ONCE), NULL, NULL, &psr_op_m, &psr_op_i, 1, tkn_a(p->tt, TOKEN(UN), "?", &tkn_ft));
    return p;
}
