...
static atg_stat add_i6_z_i6_z_i6(atg *t, gen *g, te *an, err **e) {
...
static atg_stat add_f6_o_f6_o_f6(atg *t, gen *g, te *an, err **e) {
    te *l = atg_g_g(an->d[5].p)->d[1].p, *r = atg_g_g(an->d[6].p)->d[1].p;
    if (gen_a(g, GEN_OP(ADD), gen_tmp(g, X64_TYPE(F6), t->tc++), te_c(l), te_c(r)) != GEN_STAT(OK)) return atg_err(t, an, e, __FUNCTION__);
    return ATG_STAT(OK);
}
...
static atg_stat adda_u6_e_u6_s_u6(atg *t, gen *g, te *an, err **e) {
...
static atg_stat adda_f6_e_f6_z_f6(atg *t, gen *g, te *an, err **e) {
    ...
    if (gen_a(g, GEN_OP(ADD), var_arg(g, lte, X64_TYPE(F6)), var_arg(g, lte, X64_TYPE(F6)), te_c(atg_g_g(an->d[6].p)->d[1].p)) != GEN_STAT(OK)) return atg_err(t, an, e, __FUNCTION__);
    return ATG_STAT(OK);
}
...
static atg_stat sub_u6_o_u6_s_u6(atg *t, gen *g, te *an, err **e) {
    if (gen_a(g, GEN_OP(SUB), gen_tmp(g, X64_TYPE(U6), t->tc++), te_c(atg_g_g(an->d[5].p)->d[1].p), gen_data(g, X64_TYPE(U6), ((te*) an->d[6].p)->d[4])) != GEN_STAT(OK)) return atg_err(t, an, e, __FUNCTION__);
    return ATG_STAT(OK);
}
...
const char *atg_div_inv_str = "div";

static atg_stat div_i6_e_i6_e_i6(atg *t, gen *g, te *an, err **e) {
    atg_stat stat;
    uint32_t lbl = t->lc++;
    te *l = ((te*) an->d[5].p)->d[3].p, *r = ((te*) an->d[6].p)->d[3].p, *tmp;
    if (!(l = var_arg(g, l, type_g_x64_type(l->d[2].p)))) return atg_err(t, an->d[5].p, e, "atg op l inv");
    if (!(r = var_arg(g, r, type_g_x64_type(r->d[2].p)))) return atg_err(t, an->d[5].p, e, "atg op r inv");
    if (gen_a(g, GEN_OP(NE), te_c(r), gen_data(g, X64_TYPE(I6), I6(0)), gen_lbl(g, lbl)) != GEN_STAT(OK)) return atg_err(t, an, e, __FUNCTION__);
    tmp = gen_tmp(g, X64_TYPE(M), t->tc++);
    if (gen_a(g, GEN_OP(CALL), tmp, gen_call_m(g, 2, gen_char(g, "x/0"), gen_data(g, X64_TYPE(M), P(&al_mc))), gen_data(g, X64_TYPE(M), P(mc_i_cstr))) != GEN_STAT(OK)) return atg_err(t, an, e, __FUNCTION__);
    if ((stat = atg_err_r(t, g, an, e, tmp, atg_div_inv_str)) != ATG_STAT(OK)) return stat;
    if (gen_a(g, GEN_OP(LBL), gen_lbl(g, lbl), NULL, NULL) != GEN_STAT(OK)) return atg_err(t, an, e, __FUNCTION__);
    if (gen_a(g, GEN_OP(DIV), gen_tmp(g, X64_TYPE(I6), t->tc++), l, r) != GEN_STAT(OK)) return atg_err(t, an, e, __FUNCTION__);
    return ATG_STAT(OK);
}

static atg_stat mod_i6_e_i6_s_i6(atg *t, gen *g, te *an, err **e) {
    te *l = ((te*) an->d[5].p)->d[3].p;
    if (!(l = var_arg(g, l, type_g_x64_type(l->d[2].p)))) return atg_err(t, an->d[5].p, e, "atg op l inv");
    if (gen_a(g, GEN_OP(MOD), gen_tmp(g, X64_TYPE(I6), t->tc++), l, gen_data(g, X64_TYPE(I6), ((te*) an->d[6].p)->d[4])) != GEN_STAT(OK)) return atg_err(t, an, e, __FUNCTION__);
    return ATG_STAT(OK);
}
...
void atg_arith(atg *t) {
    ...
    atg_a_o(t, OC(ADD), TYPE(F6), AST_CLS(O), TYPE(F6), AST_CLS(O), TYPE(F6), add_f6_o_f6_o_f6);
    ...
    atg_a_o(t, OC(ADD), TYPE(I6), AST_CLS(Z), TYPE(I6), AST_CLS(Z), TYPE(I6), add_i6_z_i6_z_i6);
    atg_a_o(t, OC(ADDA), TYPE(I6), AST_CLS(E), TYPE(I6), AST_CLS(S), TYPE(I6), adda_i6_e_i6_s_i6);
    atg_a_o(t, OC(ADDA), TYPE(U6), AST_CLS(E), TYPE(U6), AST_CLS(S), TYPE(U6), adda_u6_e_u6_s_u6);
    ...
    atg_a_o(t, OC(SUB), TYPE(U6), AST_CLS(O), TYPE(U6), AST_CLS(S), TYPE(U6), sub_u6_o_u6_s_u6);
    ...
    atg_a_o(t, OC(DIV), TYPE(I6), AST_CLS(E), TYPE(I6), AST_CLS(E), TYPE(I6), div_i6_e_i6_e_i6);
    atg_a_o(t, OC(MOD), TYPE(I6), AST_CLS(E), TYPE(I6), AST_CLS(S), TYPE(I6), mod_i6_e_i6_s_i6);
}
