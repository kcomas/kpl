...
static atg_stat dump_v(atg *t, gen *g, te *restrict an, err **e, gen_op go, FILE *f, size_t idnt, te *restrict type, te *restrict cd) {
...
static atg_stat dump_vd(atg *t, gen *g, te *restrict an, err **e, te *restrict cd) {
    ...
    return dump_v(t, g, an, e, go, f, 0, type, cd);
}
...
static atg_stat dump_vd_s_u5_e_te(atg *t, gen *g, te *an, err **e) {
    ...
    if ((stat = call_npr(&go, an)) != ATG_STAT(OK)) return atg_err(t, an, e, "atg inv reg prev");
    if (gen_a(g, go, gen_call_v(g, v), gen_data(g, X64_TYPE(M), P(fprintf)), NULL) != GEN_STAT(OK)) return atg_err(t, an, e, __FUNCTION__);
    for (size_t i = 2; i < type->l; i++) if ((stat = dump_v(t, g, an, e, go, f, idnt + 1, type->d[i].p, gen_idx_m(g, type_g_x64_type(type->d[i].p), 2, gen_stkv(g, X64_TYPE(M), ast_lst_tbl_e_g_i(lte)), atg_te_idx_d(g, i - 2)))) != ATG_STAT(OK)) return stat;
    go = GEN_OP(CALLV);
    if ((stat = call_npr(&go, an)) != ATG_STAT(OK)) return atg_err(t, an, e, "atg inv reg prev");
...
static atg_stat dump_vd_s_u5_e_vr(atg *t, gen *g, te *an, err **e) {
    ...
    if ((stat = dump_v(t, g, an, e, GEN_OP(CALLV), f, idnt + 1, type, gen_idx_m(g, type_g_x64_type(type), 3, gen_stkv(g, X64_TYPE(M), ast_lst_tbl_e_g_i(lte)), gen_data(g, X64_TYPE(U3), U3(sizeof(void*) * 5)), gen_tmp(g, X64_TYPE(U6), tv)))) != ATG_STAT(OK)) return stat;
