...
static chk_stat chk_op_mtch_lst_lr_b(chk *c, te *an, err **e) {
    if (chk_n(c, an->d[5].p, e) != CHK_STAT(OK)) return chk_err(c, an, e, "chk inv mtch right");
...
static chk_stat st_init(chk *c, te *an, err **e) {
    ...
    while (h) {
        ...
        if (zt->d[1].u4 == TYPE(SL)) zt = zt->d[2].p;
...
static chk_stat chk_map_vr(chk *c, te *an, err **e) {
    ...
    fa = lt->d[3].p;
    if (!fa || !fa->i->l) return chk_err(c, an, e, "chk inv fn for map");
    h = fa->i->h;
    n = h->d[0].p;
...
static chk_stat chk_vr_len(chk *c, te *an, err **e) {
    (void) e;
    an->d[3] = P(type_s_i(c->a->ta, NULL, TYPE(U6)));
    return CHK_STAT(OK);
}
...
bool chk_is_un_err(te *e) {
    if (e->d[1].u4 != TYPE(ER)) return false;
    e = e->d[2].p;
    if (e->l != 5) return false;
    return ((te*) e->d[2].p)->d[1].u4 == TYPE(SG) && ((te*) e->d[3].p)->d[1].u4 == TYPE(U6) && ((te*) e->d[4].p)->d[1].u4 == TYPE(SG);
}
...
static chk_stat chk_aply_vr(chk *c, te *an, err **e) {
    ...
    type_tbl_a(tt, c->ta, mc_i_cstr("v", c->a->ma), 1, tn);
    an->d[3] = P(type_h_i(c->ta, NULL, TYPE(UN), tt));
    return CHK_STAT(OK);
}

static chk_stat chk_cj_g_nf_t(chk *c, te *an, err **e, te **cjn) {
    te *tn;
    if (ast_g_t(*cjn, &tn) != AST_STAT(OK)) return chk_err(c, an, e, "chk cannot get type from cj");
    tn = tn->d[2].p;
...
static chk_stat chk_aply_cj(chk *c, te *an, err **e) {
    chk_stat stat;
    te *tn = an->d[4].p;
    if ((stat = chk_cj_g_nf_t(c, an, e, &tn)) != CHK_STAT(OK)) return stat;
...
static chk_stat chk_cst_cj(chk *c, te *an, err **e) {
    chk_stat stat;
    te *lt = ((te*) an->d[5].p)->d[3].p, *rt = ((te*) an->d[6].p)->d[3].p, *tn = an, *h, *n;
    ...
    while (h) {
        n = h->d[0].p;
        if (type_is_ref(((te*) n->d[2].p)->d[1].u4) && ((te*) n->d[2].p)->d[1].u4 != TYPE(UN)) {
            if ((stat = chk_move_var(c, an, e, n->d[0].p)) != CHK_STAT(OK)) return stat;
        }
        h = h->d[2].p;
    }
    return CHK_STAT(OK);
...
static chk_stat chk_cst_st(chk *c, te *an, err **e) {
    te *lt, *rt;
    if (ast_g_t(an->d[5].p, &lt) != AST_STAT(OK)) return chk_err(c, an, e, "chk cst st inv lt");
    if (ast_g_t(an->d[6].p, &rt) != AST_STAT(OK)) return chk_err(c, an, e, "chk cst st inv rt");
    if (!type_eq(lt, rt)) return chk_err(c, an, e, "chk st neq");
    an->d[3] = P(te_c(lt));
    return CHK_STAT(OK);
}
...
static chk_stat chk_op_levrroe_vr(chk *c, te *an, err **e) {
    ...
    if (rt->d[1].u4 == TYPE(SL)) rt = rt->d[2].p;
...
static chk_stat chk_uner_sg(chk *c, te *an, err **e) {
    an->d[3] = P(type_s_i(c->a->ta, NULL, TYPE(VD)));
    return chk_er_pn_fn(c, an, e);
}
...
chk *chk_b(chk *c) {
    ...
    CHK_AA(c, chk_aply_vr, AST_CLS(A), TYPE(_N), AST_CLS(A), TYPE(VR));
    ...
    CHK_AA(c, chk_map_vr, AST_CLS(O), TYPE(_N), OC(MUL), TYPE(_A), AST_CLS(O), TYPE(FN), AST_CLS(E), TYPE(VR));
    ...
    CHK_AA(c, chk_op_lr_teq_bl, AST_CLS(O), TYPE(_N), OC(NE), TYPE(_A), AST_CLS(E), TYPE(U6), AST_CLS(O), TYPE(U6));
    ...
    CHK_AA(c, chk_op_lr_teq_bl, AST_CLS(O), TYPE(_N), OC(LT), TYPE(_A), AST_CLS(E), TYPE(U6), AST_CLS(E), TYPE(U6));
    ...
    CHK_AA(c, chk_op_lr_teq, AST_CLS(O), TYPE(_N), OC(CNCT), TYPE(_A), AST_CLS(O), TYPE(SG), AST_CLS(E), TYPE(SG));
    ...
    CHK_AA(c, chk_op_levrroe_vr, AST_CLS(O), TYPE(_N), OC(CNCTA), TYPE(_A), AST_CLS(E), TYPE(VR), AST_CLS(E), TYPE(U6));
    CHK_AA(c, chk_op_levrroe_vr, AST_CLS(O), TYPE(_N), OC(CNCTA), TYPE(_A), AST_CLS(E), TYPE(VR), AST_CLS(E), TYPE(VR));
    CHK_AA(c, chk_op_levrroe_vr, AST_CLS(O), TYPE(_N), OC(CNCTA), TYPE(_A), AST_CLS(E), TYPE(VR), AST_CLS(Z), TYPE(I6));
    CHK_AA(c, chk_op_levrroe_vr, AST_CLS(O), TYPE(_N), OC(CNCTA), TYPE(_A), AST_CLS(Z), TYPE(VR), AST_CLS(S), TYPE(I6));
    CHK_AA(c, chk_op_levrroe_vr, AST_CLS(O), TYPE(_N), OC(CNCTA), TYPE(_A), AST_CLS(Z), TYPE(WVR), AST_CLS(S), TYPE(I6));
    CHK_AA(c, chk_uner_n_un, AST_CLS(O), TYPE(_N), OC(UNER), TYPE(_A), AST_CLS(_), TYPE(_N), AST_CLS(A), TYPE(UN));
    CHK_AA(c, chk_uner_n_un, AST_CLS(O), TYPE(_N), OC(UNER), TYPE(_A), AST_CLS(_), TYPE(_N), AST_CLS(O), TYPE(UN));
    CHK_AA(c, chk_uner_sg, AST_CLS(O), TYPE(_N), OC(UNER), TYPE(_A), AST_CLS(_), TYPE(_N), AST_CLS(O), TYPE(SG));
    chk_entry(c);
    chk_type(c);
    chk_z(c);
    chk_dfnagn(c);
