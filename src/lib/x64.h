...
// number of pages, assume 1 page per file
#ifndef JIT_P
    #define JIT_P (1e6 / getpagesize())
#endif
...
// mov rax, qword ptr[rdi+dsp32]
x64_stat x64_mov_rrmd(size_t *p, uint8_t *m, reg d, reg s, uint32_t dsp);
...
// add rax, word
x64_stat x64_add_rd(size_t *p, uint8_t *m, reg r, uint32_t d);
...
x64_stat x64_add_rrm(size_t *p, uint8_t *m, reg d, reg s);
...
// and rax, byte
x64_stat x64_and_rb(size_t *p, uint8_t *m, reg d, uint8_t b);
