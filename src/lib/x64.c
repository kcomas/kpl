...
static uint8_t sib(ss s2, reg d, reg s) {
    return s2 | rid(s) << 3 | rid(d);
}
...
x64_stat x64_mov_rmr(size_t *p, uint8_t *m, reg d, reg s) {
    ...
    if (d == R(SP)) return x64_a(p, m, sib(SS(00), d, d));
    return X64_STAT(OK);
}
...
x64_stat x64_mov_rrm(size_t *p, uint8_t *m, reg d, reg s) {
    ...
    if (s == R(SP)) x64_a(p, m, sib(SS(00), s, s));
    return X64_STAT(OK);
}
...
x64_stat x64_cmp_rrm(size_t *p, uint8_t *m, reg d, reg s) {
    ...
    if (s == R(SP)) return x64_a(p, m, sib(SS(00), s, s));
    return X64_STAT(OK);
}
