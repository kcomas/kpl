...
#define ZIR(N, C) x64_stat x64_##N##_ir(size_t *p, uint8_t *m, uint32_t dsp, reg r) { \
    if (r > R(15)) return X64_STAT(INV_REG); \
    uint8_t rex = REX(W); \
    if (r >= R(8)) rex |= REX(R); \
    x64_b(p, m, 3, rex, C, MOD(00) | rid(r) << 3 | RIP); \
...
ZRRMO(lea, 0x8D, d, o, s);

ZRRMOB(lea, 0x8D, d, o, s);
...
ZZRE(setbsetnaesetc, 0x0F, 0x92, 0);

ZZRE(setnbsetaesetnc, 0x0F, 0x93, 0);

ZZRE(setzsete, 0x0F, 0x94, 0);

ZZRE(setnzsetne, 0x0F, 0x95, 0);

ZZRE(setbesetna, 0x0F, 0x96, 0);

ZZRE(setnbeseta, 0x0F, 0x97, 0);

ZZRE(setlsetnge, 0x0F, 0x9C, 0);

ZZRE(setnlsetge, 0x0F, 0x9D, 0);

ZZRE(setlesetng, 0x0F, 0x9E, 0);

ZZRE(setnlesetg, 0x0F, 0x9F, 0);
