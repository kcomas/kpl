...
gen_stat idx_to(const gen *g, void *s, te *restrict ci, as *a, err **e, as_inst ai, vr *i, te *restrict from, reg rtmp, arg_id atmp) {
    ...
    if (i->l == 2) {
        if ((x == GEN_CLS(A) || x == GEN_CLS(T)) && (gen_var_g_t(i->d[0].p) == X64_TYPE(M) || gen_var_g_t(i->d[0].p) == X64_TYPE(MM)) && y == GEN_CLS(D)) {
            ...
            tgt = i->d[0].p;
            if (st_stkv_idx(st, X64_TYPE(M), tgt->d[1].u3, &idx) != GEN_STAT(OK)) return gen_err(g, ci, e, "gen stkv inv idx");
...
gen_stat idx_from(const gen *g, void *s, te *restrict ci, as *a, err **e, as_inst ai, vr *i, te *restrict to, reg rtmp, arg_id atmp) {
    ...
    if (i->l == 2) {
        ...
        if ((x == GEN_CLS(A) || x == GEN_CLS(T)) && (gen_var_g_t(i->d[0].p) == X64_TYPE(M) || gen_var_g_t(i->d[0].p) == X64_TYPE(MM)) && y == GEN_CLS(D) && (z == GEN_CLS(A) || z == GEN_CLS(T))) {
            ...
            drop_atm_kv(st, kvib, ci);
        } else if (x == GEN_CLS(V) && gen_var_g_t(i->d[0].p) == X64_TYPE(M) && y == GEN_CLS(D) && z == GEN_CLS(V)) {
            if (st->rstk->l == 0) return gen_err(g, ci, e, "gen call idx no tmp r regs");
            reg = st->rstk->d[0];
            if (arg_id_bd_g(i->d[1].p, &di) != GEN_STAT(OK)) return gen_err(g, ci, e, "gen inv data offset type");
            tgt = i->d[0].p;
