...
static gen_stat set_ax_fn(gen *g, void *s, te *ci, as *a, err **e) {
    ...
    if (gen_as(a, AS_X64(MOVQ), as_arg_i(a, ARG_ID(R), U3(kv[0]->d[2].u3)), as_arg_i(a, ARG_ID(X), U3(kv[1]->d[2].u3)), NULL, NULL, ci) != AS_STAT(OK)) return gen_err(g, ci, e, __FUNCTION__);
...
static gen_stat set_is_fn(gen *g, void *s, te *ci, as *a, err **e) {
    (void) s;
    (void) a;
    return gen_err(g, ci, e, "TODO set_is");
}
...
void gen_set(gen *g) {
    ...
    GEN_OP_A2(g, GEN_OP(SET), GEN_CLS(I), X64_TYPE(N), GEN_CLS(D), X64_TYPE(S), set_is_fn);
}
