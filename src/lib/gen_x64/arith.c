...
#define MULDIVVUVUAU(N, O) static gen_stat N##_vuvuau_fn(gen *g, void *s, te *ci, as *a, err **e) { \
    ...
    if (stk_g_idx2(s, ci->d[1].p, ci->d[2].p, &v0, &v1) != GEN_STAT(OK)) return gen_err(g, ci, e, "gen stkv inv idx"); \
    gen_as_rrmbd(a, AS_X64(MOV), R(AX), R(BP), v1, ci); \
    AS1(a, AS_X64(O), as_arg_i(a, ARG_ID(R), kv->d[2]), ci); \
...
#define AUAUDU(N, M, O) static gen_stat N##_auaudu_fn(gen *g, void *s, te *ci, as *a, err **e) { \
    gen_stat stat; \
    te *kv[2]; \
    if ((stat = get_reg_n(s, ci, kv, 2)) != GEN_STAT(OK)) return gen_err(g, ci, e, "gen reg"); \
    if (kv[0]->d[2].u3 != kv[1]->d[2].u3) AS2(a, AS_X64(MOV), as_arg_i(a, ARG_ID(R), kv[0]->d[2]), as_arg_i(a, ARG_ID(R), kv[1]->d[2]), ci); \
    uint64_t v = ((te*) ci->d[3].p)->d[1].u6; \
    if (v == 1) AS1(a, AS_X64(M), as_arg_i(a, ARG_ID(R), kv[0]->d[2]), ci); \
    else if (v <= UINT8_MAX) AS2(a, AS_X64(O), as_arg_i(a, ARG_ID(R), kv[0]->d[2]), as_arg_i(a, ARG_ID(B), U3(v)), ci); \
    else HERE("TODO"); \
    drop_atm_kv_n(s, kv, ci, 2); \
    set_code_e(ci, a); \
    return GEN_STAT(OK); \
...
#define VXVXAX(N, O) static gen_stat N##_vxvxax_fn(gen *g, void *s, te *ci, as *a, err **e) { \
    ...
    int32_t v0, v1; \
    if (stk_g_idx2(s, ci->d[1].p, ci->d[2].p, &v0, &v1) != GEN_STAT(OK)) return gen_err(g, ci, e, "gen stkv inv idx"); \
    gen_as_rrmbd(a, AS_X64(MOVSD), XMM(0), R(BP), v1, ci); \
    AS2(a, AS_X64(O##SD), as_arg_i(a, ARG_ID(X), U3(XMM(0))), as_arg_i(a, ARG_ID(X), kv->d[2]), ci); \
    gen_as_rmbdr(a, AS_X64(MOVSD), R(BP), v0, XMM(0), ci); \
    drop_atm_kv(s, kv, ci); \
    set_code_e(ci, a); \
    return GEN_STAT(OK); \
}
...
#define VXVXDX(N, O) static gen_stat N##_vxvxdx_fn(gen *g, void *s, te *ci, as *a, err **e) { \
    ...
    if (stk_g_idx2(s, ci->d[1].p, ci->d[2].p, &v0, &v1) != GEN_STAT(OK)) return gen_err(g, ci, e, "gen stkv inv idx"); \
    gen_as_rrmbd(a, AS_X64(MOVSD), XMM(0), R(BP), v1, ci); \
    AS2(a, AS_X64(O##SD), as_arg_i(a, ARG_ID(X), U3(XMM(0))), as_arg_i(a, ARG_ID(QW), ((te*) ci->d[3].p)->d[1]), ci); \
    gen_as_rmbdr(a, AS_X64(MOVSD), R(BP), v0, XMM(0), ci); \
    set_code_e(ci, a); \
    return GEN_STAT(OK); \
}
