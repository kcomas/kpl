...
static gen_stat add_auauau_fn(gen *g, void *s, te *ci, as *a, te **e)  {
    ...
    if (kv[0]->d[2].u3 == kv[2]->d[2].u3) return gen_err(GEN_STAT(INV), ci, e); /* second reg cannot be dest */
...
#define AXAXDX(N, O) static gen_stat N##_axaxdx_fn(gen *g, void *s, te *ci, as *a, te **e) { \
    (void) g; \
    gen_stat stat; \
    gen_st *st = s; \
    te *kv[2]; \
...
static gen_stat addsd_axaxax_fn(gen *g, void *s, te *ci, as *a, te **e)  {
    (void) g;
    gen_stat stat;
    gen_st *st = s;
    te *kv[3];
