...
gen_stat gen_st_p1(gen *g, gen_st *st) {
    ...
    // swap R(DX) with R(10), R(CX) with R(11), XMM(0) with XMM(8)
    static reg r[] = {R(BX), R(13), R(12), R(14), R(15), R(9), R(8), R(11), R(10), R(SI), R(DI)};
    for (size_t i = 0; i < 11; i++) vr_ab(&st->rstk, U3(r[i]));
    static reg x[] = {XMM(15), XMM(14), XMM(13), XMM(12), XMM(11), XMM(10), XMM(9), XMM(7), XMM(6), XMM(5), XMM(4), XMM(3), XMM(2), XMM(1), XMM(8)};
...
void drop_atm_kv_n(gen_st *st, te **atm_kv, const te *ci, size_t n) {
    for (size_t i = 0; i < n; i++) {
        for (size_t x = 0; x < i; x++) if (atm_kv[x] == atm_kv[i]) goto next;
        drop_atm_kv(st, atm_kv[i], ci);
        next:
    }
}
