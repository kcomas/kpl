...
void core_import_before(core_queue_item *parent) {
    parent->state_flags ^= CORE_QUEUE_ITEM_STATE(INIT);
    parent->state_flags |= CORE_QUEUE_ITEM_STATE(DEPENDENCIES);
    parent->dependencies++;
}

static error *import_string_slice(core_queue *queue, core_queue_item *parent,
        uint32_t start_idx, uint32_t end_idx) {
    char import_string[PATH_MAX] = {};
    memcpy(import_string, parent->filedata->data + start_idx, end_idx - start_idx);
    core_queue_item *dependent = core_queue_add(queue, parent->filename->data, import_string);
    if (!dependent)
...
error *core_import(core_queue *queue, core_queue_item *parent) {
    ...
    while (filedata_idx < parent->filedata->len) {
        ...
        if (parent->filedata->data[start_idx] != '"')
            return core_queue_item_error(parent, "Invalid Import Format");
        start_idx++;
        error *er = import_string_slice(queue, parent, start_idx, end_idx);
        if (er)
            return er;
    }
    parent->dependencies--;
